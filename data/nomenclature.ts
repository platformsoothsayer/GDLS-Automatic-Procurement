...
const REGISTRY = {
  ...
  "REQ.LINE": {
    ...
    fields: [
      { name: "LINE_TYPE_ID", note: "Requisition line type, goods or services" },
      { name: "ITEM_ID", note: "The part being requested" },
      { name: "QUANTITY", note: "How many, after order modifiers are applied" },
      { name: "UNIT_MEAS_LOOKUP_CODE", note: "Unit of measure the quantity is expressed in" },
      { name: "NEED_BY_DATE", note: "Date the material is required on site" },
      { name: "SUGGESTED_VENDOR_ID", note: "Supplier proposed on the line" },
      { name: "SUGGESTED_VENDOR_SITE_ID", note: "Supplier site the order would be placed against" },
      { name: "UNIT_PRICE", note: "Contract price where one exists, otherwise last paid" },
      { name: "CHARGE_ACCOUNT_ID", note: "Account the spend is charged to" },
  ...
  "REQ.IMPORT_INTERFACE": {
    key: "REQ.IMPORT_INTERFACE",
    system: "ORACLE_EBS",
    object: "PO_REQUISITIONS_INTERFACE_ALL",
    fields: [
      { name: "INTERFACE_SOURCE_CODE", note: "Marks the row as raised by the fabric rather than by a person" },
      { name: "AUTHORIZATION_STATUS", note: "Set to an unapproved state on creation. This is what keeps a human in the loop" },
      { name: "PROCESS_FLAG", note: "Whether the import has picked the row up yet" },
      { name: "REQUISITION_TYPE", note: "Purchase requisition rather than internal" },
    ],
    layer: "GOLD",
    mart: "procurement_automation_mart",
    transform: "Staged rows are written to the requisition interface and the standard requisition import creates the document in an unapproved state. No commitment exists until a person approves it in the source system.",
    assumption: "This is the integration mechanism for the pre-populated mode only. The advisory mode needs none of it. Interface access, approval hierarchy mapping and an audit trail all have to be agreed before this mode is possible.",
    verified: "NEEDS_SME_REVIEW",
  },
  ...
  "RFQ.REQUEST": {
    ...
    object: "PO_HEADERS_ALL",
    fields: [
      { name: "TYPE_LOOKUP_CODE", note: "Marks the document as a request for quotation" },
      { name: "QUOTE_TYPE_LOOKUP_CODE", note: "Whether the request is by quantity or by catalogue" },
      { name: "RFQ_CLOSE_DATE", note: "Deadline the suppliers respond by" },
      { name: "SEGMENT1", note: "Request number shown to buyers and suppliers" },
    ],
    layer: "GOLD",
    mart: "procurement_automation_mart",
    transform: "A request for quotation is assembled from the same recommendation the requisition would have used, so the two paths start from one set of numbers.",
    assumption: "Whether requests are issued through the supplier portal or by another channel needs confirming with the manufacturer's sourcing team.",
    verified: "NEEDS_SME_REVIEW",
  },
  ...
  "RFQ.QUOTATION": {
    ...
    object: "PO_LINES_ALL",
    fields: [
      { name: "UNIT_PRICE", note: "Price the supplier quoted on the line" },
      { name: "QUANTITY", note: "Quantity the supplier quoted for" },
      { name: "VENDOR_ID", note: "Supplier the quotation came from" },
      { name: "PAYMENT_TERMS_ID", note: "Terms offered with the quotation" },
    ],
    layer: "GOLD",
    mart: "supplier_performance_mart",
    transform: "Quotations are ranked on delivered feasibility first and price second. A quotation that cannot meet the need date is not eligible to win on price alone.",
    assumption: "The lead time a supplier offers on a quotation is taken at face value here. In practice it would be weighted by that supplier's delivery record.",
    verified: "NEEDS_SME_REVIEW",
  },
