...
| `PART.ORACLE_UOM` | Oracle EBS | `MTL_UNITS_OF_MEASURE_TL` | Field | Silver | `uom_code`<br>`unit_of_measure` | The primary unit held on the item is resolved to a name and normalized so quantities from different plants can be added together. | 3 Part master intelligence |
...
- `unapproved status` — The document is created unapproved and stays that way until approved
...
- **`REQ.LINE`** — Nothing is released to a supplier without a person approving it. Automation stops at the proposal. Requisition line type and supplier site are not registered on this object. If the manufacturer keys them, they need adding here first.
...
- **`DUP.CLUSTER_MEMBERSHIP`** — A cluster is a candidate for review. Nothing is merged automatically.
- **`DUP.SPEND_EXPOSURE`** — Price differences between members are treated as avoidable only where the members are the same part. Every figure is illustrative.
